...
def start_handler(message):
    ...
    if not check_join(message.from_user.id):
        safe_send(
            message.chat.id,
            f"📢 Katıl:\n{KANAL_LINK}\n{GRUP_LINK}\n\n❗ Önce katıl eksiksiz kanal ve gurup bana ait🫶, sonra menüye erişebilirsin braa!"
        )
        return
...
💎 Arkadaşlarını davet et, GÜNLÜK 2GB KAZANMAK İÇİN!

👇 Menüden seçim yap veya /komutlist yazabilirsin güveniliriz kafanda şüphe olmasın
""",
        main_menu()
    )
...
def komutlist(message):
    safe_send(
        message.chat.id,
        "📋 İşte tüm komutlar ve tıklamalı menü:",
        main_menu()
    )
...
def puan(message):
    user_id = str(message.from_user.id)
    p = DATA["points"].get(user_id, 0)
    safe_send(message.chat.id, f"🪙 Puanın: {p}", main_menu())
...
def top_users(message):
    ...
    for i, (uid, count) in enumerate(top, 1):
        name = DATA["users"].get(uid, "Anon")
        msg += f"{i}. {name} - {count}\n"
    safe_send(message.chat.id, msg, main_menu())
